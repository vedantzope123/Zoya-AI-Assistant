...
export default function App() {
  ...
  return (
    <div className="h-[100dvh] w-screen bg-[#050505] text-white flex flex-col items-center justify-between font-sans relative overflow-hidden m-0 p-0">
      ...
      <header className="absolute top-0 left-0 w-full flex justify-between items-center z-20 shrink-0 px-6 py-4 md:px-12 md:py-6">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`p-2 rounded-full transition-colors border border-white/10 ${showChat ? 'bg-violet-500/20 text-violet-300' : 'bg-white/5 text-white/70 hover:bg-white/10'}`}
            title={showChat ? "Hide Chat" : "Show Chat"}
          >
            <Send size={18} className={showChat ? "" : "rotate-90"} />
          </button>
          {messages.length > 0 && (
            <button
      ...
      <main className="absolute inset-0 flex flex-col md:flex-row items-center justify-between w-full h-full z-10 overflow-hidden pt-20 pb-24 px-4 md:px-12 pointer-events-none">
        
        {/* Left Column: Chat History */}
        <div className={`hidden md:flex flex-col w-[30%] lg:w-[25%] h-full z-10 transition-all duration-500 ${showChat ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-10 pointer-events-none'}`}>
          <div className="flex-1 overflow-y-auto scrollbar-hide pointer-events-auto pr-2 flex flex-col gap-3 mask-fade-bottom">
            {messages.map((msg) => (
              <motion.div
                key={msg.id}
                ...
                className={`max-w-[90%] p-3 rounded-2xl text-sm ${
                  msg.sender === "user" 
                    ? "bg-white/5 border border-white/10 self-end rounded-tr-none" 
                    : "bg-violet-500/10 border border-violet-500/20 self-start rounded-tl-none text-violet-100"
                }`}
              >
                <div className="text-[10px] uppercase tracking-wider opacity-40 mb-1 font-bold">
                  {msg.sender === "user" ? "You" : "Zoya"}
                </div>
                {msg.text}
              </motion.div>
            ))}
        ...
        {/* Right Column: Status & Mobile Chat */}
        <div className="flex flex-col w-full md:w-[30%] lg:w-[25%] h-full justify-center md:justify-end gap-4 z-10 pointer-events-none">
          {/* Mobile Chat Overlay */}
          <div className="md:hidden flex-1 overflow-y-auto scrollbar-hide pointer-events-auto mb-4 flex flex-col gap-2">
             {messages.slice(-5).map((msg) => (
              <motion.div
                ...
                className={`max-w-[80%] p-3 rounded-xl text-xs ${
                  msg.sender === "user" 
                    ? "bg-white/10 self-end rounded-tr-none" 
                    : "bg-violet-500/20 self-start rounded-tl-none"
                }`}
              >
                {msg.text}
              </motion.div>
          ...
          <div className="h-12 flex flex-col justify-center items-center md:items-end gap-2">
